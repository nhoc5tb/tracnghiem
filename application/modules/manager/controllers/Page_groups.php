...
class Page_groups extends Admin_Controller {
	...
	public function __construct(){ 
		...
		$this->data['controller_title'] = "Nhóm trang đơn";
	...
	public function index(){
		...
		$getcatalogs = $this->Page_group_model->getcatalogs();//lấy dữ liệu	
	...
	public function add(){
		...
		if($input){
			...
			if($this->form_validation->run()== TRUE){
				...
				if( empty($slug) ){
					$slug = $this->ruadmin->slug($this->input->post('title'),'Page_group_model');
				}else {
					$slug = $this->ruadmin->slug($this->input->post('slug'),'Page_group_model');
				}
				
				if(empty(set_value('order'))){
					$order = $this->Page_group_model->get_one_by(array("id > "=> 0),"desc");
					$order = ($order->id+1);
				}else{
				...
				$id = $this->Page_group_model->insert($setcsdl);
				if($id){
					if(empty(set_value('image_lib')) ){	
						if(empty(set_value('image_url')) ){			
							...
							if($fileimg["status"] == 200 or $fileimg["status"] == 300 ){
								$this->Page_group_model->update( $id,array('image'=>$fileimg["src"]) );	
							}
						}else{
							$fileimg = $this->ruadmin->upload_url(set_value('image_url'),$this->data['config_upload']);
							if($fileimg){
								$this->Page_group_model->update( $id,array('image'=>$fileimg) );	
							}
						}
					}else{		
						$this->Page_group_model->update( $id,array('image'=>set_value('image_lib')) );
					}
	...
	public function edit(){
		...
		try {
			...
			$this->data['getcsdl'] = $this->Page_group_model->get($id);
			...
			if($input){		
				...
				$this->form_validation->set_rules('keywords', 'Keywords', 'trim');		
				$this->form_validation->set_rules('description', 'Description', 'trim');				
				$this->form_validation->set_rules('parent_id', 'Nhóm', 'trim|callback_check_group['.$this->data['getcsdl']->id.']');	
				$this->form_validation->set_rules('order', 'Thứ tự', 'trim');
				$this->form_validation->set_rules('image-url', 'Image URL', 'trim');	
				$this->form_validation->set_rules('image', 'Hình mình họa', 'trim');			
				...
				if($this->form_validation->run()== TRUE){	
					...
					if(empty($slug))  	
						$slug = $this->ruadmin->slug(set_value('title'),'Page_group_model');	
					else{
						if($slug != $this->data['getcsdl']->slug)
							$slug = $this->ruadmin->slug(set_value('slug'),'Page_group_model');		
					}
					...
					if($this->Page_group_model->update($id,$setcsdl)){
						if(empty(set_value('image_lib')) ){	
							if(empty(set_value('image_url')) ){			
								$fileimg = $this->ruadmin->upFile($this->data['config_upload'],'image');
	...
	public function delete(){
		...
		if($this->form_validation->run() == TRUE){
			$id = set_value('id');
			$data = $this->Page_group_model->get_one_by(array('id'=>$id));			
			if ($data) {
				$post = $this->Page_model->count_by(array("id_group"=>$data->id));					
				if(!$post){
					if($this->Page_group_model->delete_by( array('id'=>$id) )){						
	...
	public function status(){
		...
		if($this->form_validation->run() == TRUE){
			...
			if ($data) {
				...
				if($this->Page_group_model->update($id,$setdata)){
