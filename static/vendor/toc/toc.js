...
function setIdFromContent(element, appendedId) {
  if (!element.id) {
    element.id = `${removeSpecialCharacters(element.innerHTML)
      .replace(/:/g, "")
      .trim()
      .toLowerCase()
      .split(" ")
...
function buildTableOfContentsFromHeadings() {
  ...
  const main = document.querySelector(".body_text");
  if (!main) {
...
function removeSpecialCharacters(str) {
  ...
  const withoutSpecialChars = str.replace(specialChars, '');

  // Loại bỏ các ký tự tiếng Việt có dấu
  const vietnameseChars = /[áàảãạâấầẩẫậăắằẳẵặéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđ]/g;
  const withoutVietnameseChars = withoutSpecialChars.replace(vietnameseChars, '');

  // Loại bỏ các ký tự không hợp lệ trong JavaScript
  const invalidChars = /[^a-zA-Z0-9_]/g;
  const withoutInvalidChars = withoutVietnameseChars.replace(invalidChars, '');
