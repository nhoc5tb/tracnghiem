...

{
...

	{
		...
		if($this->_ci->config->item('cache_dir_mobile')){
			if($this->_ci->agent->is_mobile()){
				$this->_path = $this->_ci->config->item('cache_dir').'mobile/';
			}else{
				$this->_path = $this->_ci->config->item('cache_dir').'desktop/';
			}
		}else{
...

	{
...

		{
...

			{
				...
				$cache_created = filemtime($this->_path.$this->_filename);



				// If dependency doesn't exist or is newer than this cache, delete and return FALSE

				if (! file_exists($this->_path.$dep) or filemtime($this->_path.$dep) > $cache_created)
...

	{
		...
		$cache_path = $this->_path.$this->_filename;	
...

	{
		...
		$file_path = $this->_path.$this->_filename;
